...
namespace osu_private.Updater
{
    internal class Program
    {
        private static async Task Main(string[] args)
        {
            try
            {
                ...
                Console.ReadLine();
                Console.WriteLine("osu!private関係のソフトをすべて終了します。");

                var processes = Process.GetProcessesByName("osu!private");
                foreach (var process in processes)
                {
                    process.Kill();
                }

                Console.WriteLine("osu!privateを終了しました。アップデートを開始します。");
    ...
    internal class Updater
    {
        ...
        private const string Baseurl = "https://github.com/puk06/osu-private/releases/download/";

        public async Task Update()
        {
            var downloadUrl = $"{Baseurl}{_version}/osu-private-{_arch}.zip";
